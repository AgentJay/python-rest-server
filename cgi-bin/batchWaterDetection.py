import sys, json, datetime, math, psycopg2, ee, earthEngine, urllib
...
sql = "SELECT DISTINCT landsat_wrs2.path,  landsat_wrs2.row,  st_x(st_centroid(landsat_wrs2.geom)),  st_y(st_centroid(landsat_wrs2.geom)), l8_toa_scene_count, l8_toa_cloud_stats FROM especies.landsat_wrs2 WHERE l8_toa_scene_count>0 AND path>229 ORDER BY 1,2;"
...
for pathRow in pathRows:
    ...
    thumbnail = final.getThumbUrl({
    'size': '1000',
    'region': bbox,
    'min':0,
    'max':3,
    'palette': '444444,000000,ffffff,0000ff'
    })
    print thumbnail
    urllib.urlretrieve (thumbnail, r'../../htdocs/mstmp/' + sceneid + ".jpg")