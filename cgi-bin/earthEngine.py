...
detectionExpressions = [
  ...
  {"bands":{"r":"B7", "g":"B3", "b":"B2"}, "expression":"((b('s')>(0.3402310087582287*b('v'))+0.6878785247108777)&&(b('s')<(-0.22139589479141922*b('v'))+1.0009733932691036)&&(b('s')<(114.04465599671092*b('v'))+0.38090113543454673))||((b('s')>(11.178032732580114*b('v'))-5.353961858528813)&&(b('s')<(0*b('v'))+0.9997719738616231)&&(b('s')>(-0.22139589479141922*b('v'))+1.0009733932691036))||((b('s')>(-6.555760168973008*b('v'))+0.706496211475811)&&(b('s')>(0.9781716036464391*b('v'))+0.33224042147527677)&&(b('s')<(0.3402310087582287*b('v'))+0.6878785247108777))||((b('s')>(0.19217317709040418*b('v'))+0.37128569968432384)&&(b('s')>(1.539711633483847*b('v'))+0.019193983135302795)&&(b('s')<(0.9781716036464391*b('v'))+0.33224042147527677))||((b('s')<(0.19217317709040418*b('v'))+0.37128569968432384)&&(b('s')>(-0.21980966748266745*b('v'))+0.3917513701490769)&&(b('s')>(0.4354503844174411*b('v'))+0.307720990923226))"},
  ]
...
def convertToHsv(image, sensor, bands):
    try:
        if sensor == "OLI_TIRS":  # landsat 8
            bands.setdefault('r', 'B7')  
            bands.setdefault('g', 'B5')
            bands.setdefault('b', 'B4')
            rCorrection = 1  
            gCorrection = 1
            bCorrection = 1  
        elif (sensor == "ETM+") | (sensor == "ETM"):  # landsat 7
            bands.setdefault('r', '70')  
            bands.setdefault('g', '40')
            bands.setdefault('b', '30')
            rCorrection = 10000  
            gCorrection = 10000  
            bCorrection = 10000
        elif sensor == "TM":  # landsat 5
            ...
            gCorrection = 0.02
            bCorrection = 0.02
        image = ee.Image.cat(image.select([bands['r'], bands['g'], bands['b']], ['r', 'g', 'b']))  # get the red, green, blue bands as input to the hsv
        image = image.expression("band/coeff", {'band': image.select('r', 'g', 'b'), 'coeff': [rCorrection, gCorrection, bCorrection]})  # radiometric correction for landsat
...
def detectWater(image):
    ...
    image = image.addBands(ee.call('Image.not', image.expression("b('B10')>0").mask()).clip(image.geometry()).select(["B10"], ["isEmpty"]))
    # add a band for the ndvi
    image = image.addBands(image.normalizedDifference(["B5", "B4"]).select(["nd"], ["ndvi"]))
    # add a band for the cloud mask
    if applyCloudMask:
        # add a band for areas where the temperature is low enough for cloud
        image = image.addBands(image.expression("b('B10')<" + str(lowerSceneTempThreshold)).select(["B10"], ["cloud_temp_ok"]))
        ...
        image = image.addBands(convertToHsv(image, "OLI_TIRS", {"r":"B6", "g":"B5", "b":"B4"}))
    ...
    if applyTemperatureMask:
        image = image.addBands(ee.call('Image.not', image.expression("b('B10')<" + str(upperSceneTempThreshold) + "&&b('B10')>" + str(lowerSceneTempThreshold))).select(["B10"], ["isTooHotOrCold"]))
    # add a band for the total mask
    ...
    # detect water
    for i in range(len(detectionExpressions)):
        detectionExpression = detectionExpressions[i]
        hsv = convertToHsv(image, "OLI_TIRS", detectionExpression['bands'])
